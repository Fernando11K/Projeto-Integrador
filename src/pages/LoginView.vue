    ...
    <div class="q-ma-sm shadow-24 login glossy bg-amber-7">
      <q-card style="max-width: 350px" class="bg-orange-5 fit glossy">
        ...
        <q-card-section class="row justify-center">
          ...
          <q-btn
            :disable="desabilitaBotao()"
            :loading="loading"
            class="q-mt-md glossy animate__heartBeat"
            color="dark"
            label="ENTRAR"
            @click="entrar()"
          />
    ...
    <q-card class="bg-red-14 q-pa-sm">
      <q-card-actions class="glossy shadow-24" align="center">
        <q-icon
          class="shadow-24"
          color="amber"
          name="fa-solid fa-triangle-exclamation text"
          size="25px"
        />
        <span class="q-pl-lg text-weight-medium text-amber"
    ...
    entrar() {
      ...
      usuarioService
        ...
        .then((response) => {
          ...
          if (responseData.type === "INFORMATION") {
            this.alert = true;
          } else {
            this.alert = true;
          }
